...
const getUserEmail = async function(userid) {
    let user_login = await UserLogin.find({
        where: {
            UserID: userid
        },
...
exports.REVISE = async data => {
    if (data.ti_id === null || typeof data.ti_id === undefined) {
        logger.log(
            'error',
            '/emailTemplate/Revise: No TaskInstanceID provided.'
        );
    }
    ...
    return {
        subject: `${info.number}-Revision Ready: ${
            info.assignment_display_name
        }`,
        text: `\nDeadline: ${info.due_date}\nAssignment: ${
            info.assignment_display_name
        } \n${info.task_display_name} (revision ready for review)\nCourse: (${
            info.number
        }-${info.section_number}) ${info.course_name}
    ...
    info.email
        ...
        \nTo contact the instructor: ${info.instructors.map(function(
        instructor
    ) {
...
exports.INVITE_USER = async data => {
    ...
    return {
        subject: `${
            info.number
        }: New course with Participatory Learning  – next steps`,
        text: `Hello,\n\nWelcome to the Participatory Learning system for (${
            info.number
        }-${info.section_number}) ${info.course_name} (${info.semester_name})!
...
exports.REALLOCATE = async (userid, data) => {
    if (data.ti_id === null || typeof data.ti_id === undefined) {
        logger.log(
            'error',
            '/emailTemplate/Revise: No TaskInstanceID provided.'
...
exports.REMOVE_REALLOCATE = async data => {
    if (data.ti_id === null || typeof data.ti_id === undefined) {
        logger.log(
            'error',
            '/emailTemplate/Remove Reallocate: No TaskInstanceID provided.'
        );
    }
...
exports.RESET_TASK = async data => {
    if (data.ti_id === null || typeof data.ti_id === undefined) {
        logger.log(
            'error',
            '/emailTemplate/Reset Task: No TaskInstanceID provided.'
        );
    }
...
exports.CANCEL = async data => {
    ...
    return {
        ...
        text: `\nAssignment: ${info.assignment_display_name}\n${
            info.task_display_name
        } (cancelled task) \nCourse: (${info.number}-${info.section_number}) ${
            info.course_name
        }
...
exports.NEW_REPLY = function(data) {
    return {
        ...
        text: `Dear ${
            data.name
        },\nSomeone has replied to your comment. You can view the reply here: ${
            data.link
        }\n${SUPPORT_STRING}
        `,
        html: `
            <p>Dear ${data.name},<p><br>
            <p>Someone has replied to your comment. You can view the reply here: ${
    data.link
} </p><br>
            <br><br>${SUPPORT_HTML}
        `
...
exports.NEW_VOLUNTEER = function(data) {
    return {
        ...
        text: `Dear ${
            data.name
        },\nA student has made a volunteer request. You can view the request here: ${
            data.link
        }\n${SUPPORT_STRING}
        `,
        html: `
            <p>Dear ${data.name},<p><br>
            <p>A student has made a volunteer request. You can view the request here: ${
    data.link
} </p><br>
            <br><br>${SUPPORT_HTML}
        `
...
exports.NEW_FLAG = function(data) {
    return {
        ...
        text: `Dear ${
            data.name
        },\nSomeone has flagged a comment. You can view the flag here: ${
            data.link
        }\n${SUPPORT_STRING}
        `,
        html: `
            <p>Dear ${data.name},<p><br>
            <p>Someone has flagged a comment. You can view the flag here: ${
    data.link
} </p><br>
            <br><br>${SUPPORT_HTML}
        `
