...
// Endpoint to get assignment instance report
  ...
  router.get('/TaskStatus/:assignmentInstanceID', async function(req, res) {
  var TaskStatus = {};
  var ai = await AssignmentInstance.find({
    where: {
      AssignmentInstanceID: req.params.assignmentInstanceID
  ...
  await Promise.map(JSON.parse(wi.TaskCollection), async function(ti){
      ...
      for (var i=0; i < ti.length; i++) {
        if(!TaskStatus.hasOwnProperty(ti[i].UserID)){
          TaskStatus[ti[i].UserID] = [ti[i].TaskInstanceID];
        }
        else{
            TaskStatus[ti[i].UserID].push(ti[i].TaskInstanceID);
            TaskStatus[ti[i].UserID] = TaskStatus[ti[i].UserID].filter( function( item, index, inputArray ) {
             return inputArray.indexOf(item) == index;
            });
          }
          res.json({
              "UserID": TaskStatus[ti[i].UserID]
          });
          return
      }
      });
