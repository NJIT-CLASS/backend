...
REST_ROUTER.prototype.handleRoutes = function (router) {
    ...
    */
    ...
    //------------------------------------------------------------------------------------------
    router.get('/comments/accessLink/:originTaskInstanceID', async function(req, res) {

           var taskFactory = new TaskFactory();
           var FinalTargetID = req.params.originTaskInstanceID;
           await taskFactory.getNextTask(req.params.originTaskInstanceID, new Array()).then(function(NextTask) {
      ...
      Notifications.findAll({
        ...
        /*  var sortedRows = [];
            ...
            for (var i = 0; i < rows.length; i++) {
              if (!input.includes(rows[i].CommentsID)) {
                input.push(rows[i].CommentsID);
              }
            }
            for (var i = 0; i < input.length; i++) {
              var temp = [];
                for (var j = 0; j < rows.length; j++) {
                    if (input[i] == rows[j].CommentsID){
                      temp.push(rows[j]);
                    }
                }
                sortedRows.push(temp);
            } */
        }).catch(function(err) {
