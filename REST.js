...
REST_ROUTER.prototype.handleRoutes = function(router) {
    ...
    router.use(async function(req, res, next) {
        ...
        let insertAPIResult = await sequelize.query(
            " INSERT INTO apistatistics (StartTime, Route) VALUES(NOW(6), :route) ",
            {
                replacements: {
                    route: path
                }
    ...
    router.use(function(req, res, next) {
        ...
        res.json = function() {
            sequelize.query(
                " UPDATE apistatistics SET EndTime = NOW(6) WHERE StatID = :statID",
                {
                    replacements: {
                        statID: statID
                    }
        ...
        UserLogin.find({
            ...
            .then(async function(user) {
                ...
                if (user == null) {
                    ...
                    // deny if user is manually blocked
                    ...
                    user.Timeout != null &&
                    user.Timeout > current_timestamp
                ) {
                    // deny if there is a timeout in the future
                    console.log(
                        "/login: prevented login due to timeout for " +
                            user.Email
                    );
                    ...
                    if (
                        ...
                        UserLogin.update(
                            ...
                            .then(function(userLogin) {
                                ...
                                refreshTokens[refreshToken] = [
                                    new Date().addDays(REFRESH_TOKEN_LIFE),
                                    user.UserID
                                ];
                        ...
                        if (attempts >= 5) {
                            console.log(
                                "/login: setting new timeout for " + user.Email
                            );
                            switch (attempts) {
                                case 5:
                                    minutes = 1;
        ...
        return UserLogin.findOne({
            ...
            .then(async user => {
                ...
                user.save().then(result => {
                    ...
                    let email = new Email();
                    email.sendNow(result.UserID, "reset password", {
                        pass: temp_pass,
                        email: req.body.email
                    });
                    res.status(200).end();
    ...
    router.post("/addInitialUser", async function(req, res) {
        ...
        return sequelize
            .query(
                "CALL addInitialUserToSystem (:firstName,:lastName,:Instructor,:Admin,:Role,:Email,:Phone,:Password,:Pending );",
                {
                    replacements: {
            ...
            .then(function(queryResult) {
                let email = new Email();
                email.sendNow(queryResult[0].UserID, "onboarding", {
                    email: req.body.email
                });
    ...
    router.put("/update/email", function(req, res) {
        ...
        UserLogin.find({
            ...
            if (
                user != null &&
                (await password.verify(user.Password, req.body.password))
            ) {
    ...
    router.get("/generalUser/:userid", function(req, res) {
        User.find({
            ...
            .then(function(user) {
                ...
                var org_group =
                    typeof user.OrganizationGroup == "string"
                        ? JSON.parse(user.OrganizationGroup)
    ...
    router.post("/task/reset", teacherAuthentication, async function(req, res) {
        ...
        if (
            req.body.keep_content === null ||
            typeof req.body.keep_content === undefined
        ) {
        ...
        await trigger.reset(
            req.body.ti_id,
            req.body.duration,
            req.body.keep_content
        );
        res.status(200).end();
    });

    router.post("/assignment/create", teacherAuthentication, function(
        req,
        res
    ) {
        logger.log(
            "info",
            "/assignment/create: Initializing assignment formulation process...",
            {
        ...
        if (req.body.partialAssignmentId == null) {
            PartialAssignments.create({
                ...
                .then(result => {
                    taskFactory
                        .createAssignment(req.body.assignment)
                        .then(function(done) {
                            if (done) {
                                res.json({
                                    Error: false,
                                    PartialAssignmentID:
                                        result.PartialAssignmentID
                                });
                            } else {
                                res.status(400).end();
                            }
    ...
    router.post("/gradeReport", participantAuthentication, async function(
        req,
        res
    ) {
        let grade = new Grade();
    ...
    router.post(
        ...
        async function(req, res) {
            ...
            await Promise.mapSeries(ais, async ai => {
                report[
                    ai.AssignmentInstanceID
                ] = await grade.getAssignmentGradeReport(
                    ai.AssignmentInstanceID
                );
            });
    ...
    //---------------------------------------------------------------------------
    router.get("/notifications/all", participantAuthentication, function(
        req,
        res
    ) {
    ...
    //---------------------------------------------------------------------------
    router.get(
        "/notifications/dismiss/:notificationsID",
        participantAuthentication,
        function(req, res) {
            ...
            Notifications.update(
                ...
                .catch(function(err) {
                    console.log(
                        "/notifications/dismiss/:notificationsID" + err.message
                    );
                    res.status(400).end();
                });
        }
    ...
    router.get("/partialAssignments/byId/:partialAssignmentId", function(
        req,
        res
    ) {
    ...
    router.get("/partialAssignments/duplicate/:partialAssignmentId", function(
        req,
        res
    ) {
        var newid;
        logger.log(
            "info",
            "get: /partialAssignments/duplicate/:partialAssignmentId",
            {
                req_query: req.query,
                req_params: req.params
            }
        );
        PartialAssignments.max("PartialAssignmentID")
            .then(max => {
                newid = max + 1;
            })
            .then(
                PartialAssignments.find({
                    ...
                    data.AA_name =
                        result.PartialAssignmentName + "-copy-" + newid;
                    data.AA_display_name =
                        result.PartialAssignmentName + "-copy-" + newid;
    ...
    router.get(
        "/partialAssignments/duplicate/:partialAssignmentId/:CourseID",
        function(req, res) {
            var newid;
            PartialAssignments.max("PartialAssignmentID")
                .then(max => {
    ...
    router.get("/getAssignments/:courseId", participantAuthentication, function(
        req,
        res
    ) {
    ...
    router.get("/getActiveAssignmentsForSection/:sectionId", function(
        req,
        res
    ) {
        logger.log(
            "info",
            `/getActiveAssignmentsForSection/:sectionId: Finding Assignments for Section ${
    ...
    router.get("/getActiveAssignments/:courseId", function(req, res) {
        ...
        Assignment.findAll({
            ...
            .catch(function(err) {
                console.log(
                    "/getActiveAssignments/" + req.params.courseId + ": " + err
                );
                res.status(400).end();
            });
    });

    router.post("/files/upload/:type?", participantAuthentication, function(
        req,
        res
    ) {
                ...
                case "task":
                    return TaskInstance.find({
                        ...
                        .catch(er => {
                            logger.log(
                                "error",
                                "file task info not uploaded successfully",
                                er
                            );
                            res.status(400).end();
                        });
                ...
                case "profile-picture":
                    UserContact.update(
                        ...
                        .then(function(done) {
                            logger.log(
                                "info",
                                "user updated with new profile pictures info",
                                {
                                    res: done
                                }
                            );
                        ...
                        .catch(er => {
                            logger.log(
                                "error",
                                "profile-picture info not uploaded successfully",
                                er
                            );
                            res.status(400).end();
                        });
    ...
    router.post("/file/upload/:type?", participantAuthentication, function(
        req,
        res
    ) {
        console.log("File upload:", req.body);
                ...
                switch (req.params.type) {
                    case "task":
                        TaskInstance.find({
                            ...
                            .then(ti => {
                                let newFilesArray = JSON.parse(ti.Files) || [];
                                newFilesArray = newFilesArray.concat([
                                    result.FileID
                                ]);
    ...
    router.get("/file/download/:fileId", participantAuthentication, function(
        req,
        res
    ) {
    ...
    router.delete(
        ...
        async function(req, res) {
            ...
            logger.log(
                "info",
                "deleting file info from database with FileID: ",
                req.params.fileId,
                req.body
            );
            FileReference.findOne({
                ...
                .catch(function(err) {
                    logger.log(
                        "error",
                        "file info not deleted successfully",
                        err
                    );
                    res.status(400).end();
                });
    ...
    router.get(
        ...
        function(req, res) {
            SectionUser.findAll({
                ...
                console.log(
                    `/getEnrolledCourses/ Courses for ${
    ...
    router.get(
        "/getActiveEnrolledSections/:courseID",
        participantAuthentication,
        function(req, res) {
            SectionUser.findAll({
                where: {
                ...
                Course.find({
                    ...
                    console.log(
                        `/getActiveEnrolledSections/ Courses for ${
                            req.query.studentID
                        } found `
                    );
                    res.json({
    ...
    router.get(
        "/taskInstanceTemplate/main/:taskInstanceID",
        participantAuthentication,
        function(req, res) {
            logger.log(
                "info",
                "get: /taskInstanceTemplate/main/:taskInstanceID",
                {
                    req_query: req.query
                }
            );
            TaskInstance.find({
                ...
                .then(function(taskInstanceResult) {
                    return res.json({
                        ...
                        courseName:
                            taskInstanceResult.AssignmentInstance.Section.Course
                                .Name,
                        courseNumber:
                            taskInstanceResult.AssignmentInstance.Section.Course
                                .Number,
                        ...
                        semesterID:
                            taskInstanceResult.AssignmentInstance.Section
                                .Semester.SemesterID,
                        semesterName:
                            taskInstanceResult.AssignmentInstance.Section
                                .Semester.Name,
                        sectionName:
                            taskInstanceResult.AssignmentInstance.Section.Name,
                        sectionID:
                            taskInstanceResult.AssignmentInstance.Section.Name
                    });
                });
    ...
    router.post(
        "/taskInstanceTemplate/create/submit",
        participantAuthentication,
        async function(req, res) {
            var grade = new Grade();
            var trigger = new TaskTrigger();
            ...
            if (
                ti.TaskActivity.Type === "edit" ||
                ti.TaskActivity.Type === "comment"
            ) {
                await trigger.approved(
                    req.body.taskInstanceid,
                    req.body.taskInstanceData
                );
            } else {
                ...
                if (ti.TaskActivity.Type === "grade_problem") {
                    final_grade = await trigger.finalGrade(
                        ti,
                        req.body.taskInstanceData
                    );
                }
    ...
    router.post(
        "/taskInstanceTemplate/create/save",
        participantAuthentication,
        async function(req, res) {
            if (req.body.taskInstanceid == null) {
                console.log(
                    "/taskInstanceTemplate/create/save : TaskInstanceID cannot be null"
                );
                res.status(400).end();
                return;
            }
            if (req.body.UserID == null) {
                console.log(
                    "/taskInstanceTemplate/create/save : UserID cannot be null"
                );
                res.status(400).end();
                return;
            }
            if (req.body.taskInstanceData == null) {
                console.log(
                    "/taskInstanceTemplate/create/save : Data cannot be null"
                );
                res.status(400).end();
                return;
            }
            ...
            if (req.body.UserID != ti.UserID) {
                console.log(
                    "/taskInstanceTemplate/create/save : UserID Incorrect Match"
                );
                res.status(400).end();
                return;
            }
    ...
    router.get(
        "/getPendingTaskInstances/:userID",
        participantAuthentication,
        function(req, res) {
            TaskInstance.findAll({
                ...
                .then(function(taskInstances) {
                    console.log(
                        "/getPendingTaskInstances/ TaskInstances found"
                    );
                    res.json({
                        Error: false,
    ...
    router.get(
        "/getCompletedTaskInstances/:userID",
        participantAuthentication,
        function(req, res) {
            TaskInstance.findAll({
                where: {
                ...
                .then(function(taskInstances) {
                    console.log(
                        "/getCompletedTaskInstances/ TaskInstances found"
                    );
    ...
    router.get(
        "/getAssignmentRecord/:assignmentInstanceid",
        participantAuthentication,
        function(req, res) {
            // var taskFactory = new TaskFactory();

            console.log(
                "/getAssignmentRecord/:assignmentInstanceid: Initiating..."
            );
                    ...
                    return Assignment.find({
                        ...
                        return User.find({
                            ...
                            return Course.find({
                                ...
                                console.log(
                                    "/getAssignmentRecord/:assignmentInstanceid: Done!"
                                );
    ...
    router.get("/SectionsByUser/:userId", participantAuthentication, function(
        req,
        res
    ) {
        SectionUser.findAll({
            ...
            SectionUser.findAll({
                ...
                SectionUsers = SectionUsers.map(user => {
                    ...
                    if (user.User.VolunteerPools.length != 0) {
                        ...
                        user.Volunteer != 0 &&
                        user.Volunteer != "Declined" &&
                        user.Volunteer != "Removed"
                    ) {
    ...
    router.post(
        "/sectionUsers/addMany/:sectionid",
        teacherAuthentication,
        function(req, res) {
            //expects - users
            return Promise.mapSeries(req.body.users, async function(
                userDetails
            ) {
                let role = null;
                ...
                return sequelize
                    .query(
                        "CALL addUserToSection (:FirstName,:LastName,:Instructor,:Admin,:Role,:Email,:Phone,:Password,:Pending,:SectionID,:Active,:Volunteer,:SectionRole )",
                        {
                            replacements: {
                                FirstName: userDetails.firstName || "",
                                LastName: userDetails.lastName || "",
                                Instructor:
                                    userDetails.role === "Instructor" ? 1 : 0,
                                Admin: 0,
                                Role: role,
                    ...
                    .catch(function(err) {
                        ...
                        logger.log(
                            "error",
                            "post: sectionUsers/:sectionid, user invited to system",
                            {
                                req_body: req.body,
                                firstName: userDetails.firstName,
                                error: err
                                                    ...
                                                    .then(function(userLogin) {
                                                        ...
                                                        return SectionUser.create(
                                                            ...
                                                            .then(function(
                                                                ...
                                                                console.log(
                                                                    "Creating user, inviting, and adding to section"
                                                                );
                                                                logger.log(
                                                                    "info",
                                                                    "post: sectionUsers/:sectionid, user invited to system",
    ...
    router.post(
        "/sectionUsers/changeActive/:sectionUserID",
        teacherAuthentication,
        (req, res) => {
            ...
            SectionUser.update(
                ...
                .catch(err => {
                    logger.log(
                        "error",
                        "post: /sectionUser/changeActive/, user active status not set",
                        {
                            error: err,
                            req_params: req.params
                        }
...
*/
    router.get("/getWorkflow/:ti_id", participantAuthentication, async function(
        req,
        res
    ) {
        var ti = await TaskInstance.find({
    ...
    router.get("/task/files/:taskId", participantAuthentication, async function(
        req,
        res
    ) {
        let result = await TaskInstance.findOne({
    ...
    router.get("/allocate", teacherAuthentication, function(req, res) {
        ...
        Promise.all([allocator.getUser(1)])
            .then(function(done) {
                console.log(done[0]);
            })
            .then(function() {
                Promise.all([allocator.getUser(2)])
                    .then(function(done) {
                        console.log(done[0]);
                    })
                    .then(function() {
                        Promise.all([allocator.getUser(3)])
                            .then(function(done) {
                                console.log(done[0]);
                            })
                            .then(function() {
                                Promise.all([allocator.getUser(4)])
                                    .then(function(done) {
                                        console.log(done[0]);
                                    })
                                    .then(function() {
                                        Promise.all([allocator.getUser(5)])
                                            .then(function(done) {
                                                console.log(done[0]);
                                            })
                                            .then(function() {
    ...
    router.get(
        "/sendEmailNotification/:taskInstanceId",
        participantAuthentication,
        function(req, res) {
            var email = new Email();
    ...
    router.post(
        "/sectionUser/inactivate/:section_user_id",
        teacherAuthentication,
        function(req, res) {
            logger.log(
                "info",
                "post: /sectionUser/inactivate/, inactivate section user",
                {
                    req_body: req.body,
                    req_params: req.params
                }
            ...
            var section_user_id =
                req.body.section_user_id || req.params.section_user_id;

            if (section_user_id == null) {
                logger.log(
                    "info",
                    "section_user_id is required but not specified"
                );
                return res.status(400).end();
            }
    ...
    router.post("/getUserAssignmentGrades", participantAuthentication, function(
        req,
        res
    ) {
        if (req.body.UserID == null || req.body.sectionID == null) {
            console.log(req);
            console.log(
                "/getUserAssignmentGrades:userID : no user or section ID passed"
            );
            res.status(400).end();
            return;
        }
                ...
                return Promise.map(response, function(sectionIDs) {
                    ...
                    return AssignmentGrade.find({
                        ...
                        return AssignmentInstance.find({
                            where: {
                                AssignmentInstanceID:
                                    gradesJSON.AssignmentInstanceID
                            },
    ...
    router.get("/getAssignmentGrade/:ai_id", async function(req, res) {
        if (req.params.ai_id == null) {
            console.log(
                "/getAssignmentGrade/:ai_id : assignmentInstanceID cannot be null"
            );
            res.status(400).end();
            return;
        }
    ...
    router.post("/createSemester", teacherAuthentication, function(req, res) {
        ...
        if (req.body.end_sem == null || req.body.start_sem == null) {
            ...
            console.log(
                "/createSemester : StartDate cannot be grater than EndDate"
            );
            res.status(400).end();
        } else {
            Semester.find({
    ...
    router.get("/semester/:semesterid", participantAuthentication, function(
        req,
        res
    ) {
        Semester.find({
    ...
    router.post("/course/createsection", teacherAuthentication, function(
        req,
        res
    ) {
        ...
        UserContact.create({
            ...
            User.create({
                ...
                UserLogin.create({
                    ...
                    .then(function(userLogin) {
                        console.log(
                            "/user/create: New user added to the system"
                        );
    ...
    router.post("/adduser", teacherAuthentication, function(req, res) {
        ...
        var organization =
            "organization" in req.body ? req.body.organization : null;
                                ...
                                UserContact.create({
                                    ...
                                    .then(async function(userCon) {
                                        ...
                                        UserLogin.create({
                                            ...
                                            Pending: req.body.trustpassword
                                                ? false
                                                : true
                                        })
                                            .catch(function(err) {
    ...
    router.get("/course/:courseId", participantAuthentication, function(
        req,
        res
    ) {
        Course.find({
    ...
    router.post("/course/updatesection", teacherAuthentication, function(
        req,
        res
    ) {
        if (req.body.sectionid == null) {
    ...
    router.delete("/course/deleteuser", teacherAuthentication, function(
        req,
        res
    ) {
        SectionUser.destroy({
    ...
    router.get(
        ...
        async function(req, res) {
            ...
            let createdCourses = await Course.findAll({
                ...
                logger.log(
                    "error",
                    "failed getting courses created information",
                    {
                        error: err
                    }
                );
    ...
    //Get All Instructors
    router.get("/instructor/all", participantAuthentication, function(
        req,
        res
    ) {
        User.findAll({
    ...
    router.post("/createorganization", teacherAuthentication, function(
        req,
        res
    ) {
    ...
    router.get("/taskInstance/:userid", participantAuthentication, function(
        req,
        res
    ) {
        TaskInstance.findAll({
    ...
    router.post("assignment/section", participantAuthentication, function(
        req,
        res
    ) {
        ...
        async function(req, res) {
            ...
            await TaskInstance.find({
                ...
                .then(async function(current_ti) {
                    ...
                    if (
                        JSON.parse(ti.Status)[1] == "cancelled" ||
                        JSON.parse(ti.Status)[0] == "bypassed"
                    ) {
                        logger.log(
                            "info",
                            " Algorithm Cancelled returning error"
                        );
                        ViewTask = 0;
                        res.json({
                            error: true,
                    ...
                    if (JSON.parse(ti.Status)[0] == "not_yet_started") {
                        logger.log(
                            "info",
                            " Algorithm 1  returning denided access"
                        );
                        ViewTask = 0;
                        res.json({
                            error: true,
                    ...
                    if (
                        JSON.parse(ti.Status)[0] == "started" &&
                        ti.UserID != current_user_id &&
                        !user.Admin
                    ) {
                        logger.log(
                            "info",
                            " Algorithm  2 returning denied access"
                        );
                        ViewTask = 0;
                        res.json({
                            error: true,
                    ...
                    if (ti.TaskActivity.SeeSibblings) {
                        ...
                        await Promise.map(ti_temp, async ti => {
                            ...
                            if (
                                ti.TaskInstanceID != req.params.taskInstanceId
                            ) {
                                sibling_ti.push(ti);
                            }
                        });
                    ...
                    var view_constraint = await allocator.View_Access(
                        res,
                        current_user_id,
                        current_ti,
                        MultipleUsers,
                    ...
                    if (!BlockedView && ViewTask) {
                        ...
                        if (PathLength > 1) {
                            ...
                            await allocator.SetDataVersion(
                                current_ti,
                                view_constraint.WhichVersion
                            ); // set version on current task if its not first task
                            ...
                            for (var t = PathLength - 2; t >= 0; t--) {
                                // Go in reverse Order
                                var task = fullPath[t];
                                var prev_tis = [];
                                if (task.constructor === Array) {
                                    // if there were siblings add each to array
                                    await Promise.map(
                                        task,
                                        async taskInstance => {
                                ...
                                await Promise.map(prev_tis, async pre_ti => {
                                    // for each sibling or single task
                                    /* Skip Tasks that were Cancelled  */
                                    if (
                                        JSON.parse(pre_ti.Status)[1] !=
                                        "cancelled"
                                    ) {
                                        view_constraint = await allocator.View_Access(
                                            res,
                                            current_user_id,
                                            pre_ti,
                                            [],
                        ...
                        if (newStatus[4] === "not_opened") {
                            ...
                            logger.log(
                                "info",
                                "task opened for the first time, updating status..."
                            );
                            await TaskInstance.update(
                                {
                                    Status: JSON.stringify(newStatus)
                    ...
                    if (BlockedView) {
                        ...
                        await Promise.map(PendingTaskInstances, async ti => {
                            if (
                                BlockableTAs.includes(
                                    ti.TaskActivity.TaskActivityID
                                )
                            ) {
                        ...
                        res.json({
                            error: true,
                            message:
                                "You cannot work on this task until you have completed your task(s): " +
                                Ta_Names
                        });
                        return;
    ...
    */
    ...
    router.get("/getAssignToSection/", teacherAuthentication, async function(
        req,
        res
    ) {
        ...
        await Promise.all(workflowActivity)
            .then(async function(result) {
                ...
                if (result !== null || typeof result !== undefined) {
                    ...
                    await result.forEach(function(workflow) {
                        ...
                        workflowNames[workflow.WorkflowActivityID] =
                            workflow.Name;
                    });
                }
            ...
            .then(async function(resultArray) {
                ...
                return Promise.map(resultArray[1], function(workflow) {
                    console.log(
                        "WorkflowActivityID: ",
                        workflow.WorkflowActivityID
                    );

                    //Loop through TaskActivityCollection in each workflowActivity
                    console.log(
                        "workflow.TaskActivityCollection",
                        workflow.TaskActivityCollection
                    );
                    return Promise.map(
                        JSON.parse(workflow.TaskActivityCollection),
                        function(taskActivityID) {
                            ...
                            return TaskActivity.find({
                                ...
                                .then(function(taskActivity) {
                                    ...
                                    taskCollection[
                                        ...
                                        var x =
                                            a.taskActivityID < b.taskActivityID
                                                ? -1
                                                : 1;
                                        return x;
                                    });
    ...
    router.post(
        ...
        async function(req, res) {
            ...
            console.log(
                "/getAssignToSection/submit/  Creating Assignment Instance..."
            );
            ...
            await taskFactory
                .createAssignmentInstances(
                    ...
                    req.body.startDate,
                    req.body.wf_timing,
                    req.body.assignmentDisplayName
                )
                .then(async function(done) {
                    ...
                    if (moment(req.body.startDate) <= new Date()) {
                        await Promise.mapSeries(
                            req.body.sectionIDs,
                            async function(secId) {
                                await Promise.mapSeries(done, async function(
                                    assignmentInstanceId
                                ) {
                                    console.log(
    ...
    router.get(
        "/openRevision/:taskInstanceID",
        participantAuthentication,
        function(res, req) {
            if (req.params.taskInstanceID == null) {
                console.log(
                    "/openRevision/:taskInstanceID TaskInstanceID cannot be empty!"
                );
                res.stats(400).end();
            }
    ...
    router.get("/openRevision/save", participantAuthentication, function(
        res,
        req
    ) {
    ...
    router.get("/openRevision/submit", participantAuthentication, function(
        res,
        req
    ) {
        if (req.body.data == null) {
    ...
    router.post(
        ...
        async (req, res) => {
            ...
            let response = await a.reallocate_user_to_task(
                ti,
                req.body.user_id,
                req.body.isExtraCredit
            );
    ...
    router.post(
        ...
        async (req, res) => {
            ...
            let response = await ra.reallocate_tasks(
                req.body.tasks,
                req.body.users,
                req.body.sectionID,
                undefined,
    ...
    //Endpoint to return Semester Information
    router.get(
        "/getOrganizationSemesters/:organizationID",
        participantAuthentication,
        function(req, res) {
            Semester.findAll({
                where: {
    ...
    router.get("/section/:sectionid", participantAuthentication, function(
        req,
        res
    ) {
        Section.find({
    ...
    router.get("/course/delete/:courseid", teacherAuthentication, function(
        req,
        res
    ) {
        Course.destroy({
    ...
    router.get("/semester/delete/:semesterid", teacherAuthentication, function(
        req,
        res
    ) {
        Semester.destroy({
    ...
    router.get("/section/delete/:sectionid", teacherAuthentication, function(
        req,
        res
    ) {
        Section.destroy({
    ...
    //Endpoint to update a course
    router.post("/course/update/:courseid", teacherAuthentication, function(
        req,
        res
    ) {
        if (req.body.Number == null) {
    ...
    router.post("/semester/update/:semesterid", teacherAuthentication, function(
        req,
        res
    ) {
        if (req.body.Name == null) {
    ...
    router.delete("/delete/user/:userID", teacherAuthentication, (req, res) => {
        ...
        return sequelize
            ...
            .then(result => {
                logger.log(
                    "info",
                    "post: /delete/user, user deleted from system",
                    {
                        req_params: req.params
                    }
                );
    ...
    router.get(
        "/EveryonesWork/:assignmentInstanceID",
        participantAuthentication,
        async function(req, res) {
            var everyones_work = {};
            ...
            await Promise.map(JSON.parse(ai.WorkflowCollection), async function(
                wi
            ) {
                ...
                await Promise.map(JSON.parse(wi.TaskCollection), async function(
                    ti
                ) {
                    ...
                    for (var i = 0; i < ti.length; i++) {
                        if (!everyones_work.hasOwnProperty(ti[i].UserID)) {
                            everyones_work[ti[i].UserID] = [
                                ti[i].TaskInstanceID
                            ];
                        } else {
                            everyones_work[ti[i].UserID].push(
                                ti[i].TaskInstanceID
                            );
                            everyones_work[ti[i].UserID] = everyones_work[
                                ti[i].UserID
    ...
    router.get(
        "/EveryonesWork/alternate/:assignmentInstanceID",
        participantAuthentication,
        async function(req, res) {
            console.log(
                "/EveryonesWork/alternate/:assignmentInstanceID: was called"
            );
    ...
    //---------------------------------------------------------------------------
    router.get(
        "/EveryonesWork/AssignmentInstanceID/:assignmentInstanceID",
        participantAuthentication,
        async function(req, res) {
            console.log(
                "/EveryonesWork/AssignmentInstanceID/:assignmentInstanceID: was called"
            );
            ...
            Promise.map(wa, async workflowAct => {
                ...
                var workflowInstances = wI.map(async wI => {
                    ...
                    var lastTask = await TaskInstance.max("TaskInstanceID", {
                        where: {
                            AssignmentInstanceID:
                                req.params.assignmentInstanceID,
                            WorkflowInstanceID: wI.WorkflowInstanceID,
                            Status: {
                                $like: '%"complete"%'
                            }
    ...
    router.post("/approved", participantAuthentication, async function(
        req,
        res
    ) {
        var trigger = new TaskTrigger();
    ...
    router.get(
        "/getAssignmentReport/:assignmentInstanceID",
        participantAuthentication,
        function(req, res) {
            ...
            AssignmentInstance.findOne({
                ...
                try {
                    ...
                    Promise.all(finalResults.map(Promise.all, Promise))
                        .then(arrArr => {
                            return res.json({
                                Result: arrArr
                            });
    ...
    router.get(
        "/getAssignmentReport/alternate/:assignmentInstanceID",
        participantAuthentication,
        (req, res) => {
            ...
            let fetchTask = taskInstanceID => {
                return new Promise(function(resolve, reject) {
                    TaskInstance.findOne({
                        ...
                        .then(tiData => {
                            const waID =
                                tiData.WorkflowInstance.WorkflowActivityID;
                            const wiID =
                                tiData.WorkflowInstance.WorkflowInstanceID;
                            const taID = tiData.TaskActivity.TaskActivityID;

                            if (
                                !assignmentObject[waID].WorkflowInstances[wiID]
                            ) {
                                assignmentObject[waID].WorkflowInstances[
                                    wiID
                                ] = {};
                            }
            ...
            let fetchWorkflow = workflowArray => {
                return workflowArray.map(workflow => {
                    return WorkflowInstance.find({
                        ...
                        .then(result => {
                            assignmentObject[
                                result.WorkflowActivity.WorkflowActivityID
                            ] = {
                                WorkflowInstances: {},
                                Structure:
                                    result.WorkflowActivity.WorkflowStructure,
    ...
    router.get(
        "/sectionUserInfo/:userId/:sectionId",
        participantAuthentication,
        function(req, res) {
            SectionUser.findOne({
                where: {
    ...
    router.get("/VolunteerPool/countOfUsers", teacherAuthentication, function(
        req,
        res
    ) {
    ...
    router.get(
        "/VolunteerPool/VolunteersInSection/:SectionID",
        teacherAuthentication,
        function(req, res) {
    ...
    router.get(
        "/VolunteerPool/UserInPool/:UserID",
        teacherAuthentication,
        function(req, res) {
    ...
    router.post(
        "/VolunteerPool/sectionlStatusUpdate/",
        teacherAuthentication,
        function(req, res) {
            VolunteerPool.update(
                ...
                .catch(function(err) {
                    console.log(
                        "/VolunteerPool/sectionlStatusUpdate " + err.message
                    );
                    res.status(400).end();
                });
        }
    ...
    router.post(
        "/VolunteerPool/assignmentInstanceStatusUpdate/",
        teacherAuthentication,
        function(req, res) {
            VolunteerPool.update(
                {
    ...
    router.get("/reallocatepools/:ai_id", teacherAuthentication, function(
        req,
        res
    ) {
        ...
        if (pools[volunteer_pool] == null) {
            ...
            if (pools[section_students] == null) {
                ...
                return Promise.each(pools[section_instructors], function(si) {
                    return reallocate.reallocate(
                        si,
                        pools[section_instructors]
                    );
                });
    ...
    router.post("/reallocate_ai/", teacherAuthentication, async function(
        req,
        res
    ) {
        var reallocate = new Allocator();
    ...
    router.post("/comments/add", function(req, res) {
        ...
        if (
            req.body.UserID === null ||
            (req.body.TaskInstanceID === null &&
                req.body.AssignmentInstanceID === null) ||
            (req.body.CommentsText === null && req.body.Rating === null) ||
            req.body.ReplyLevel === null
        ) {
                            ...
                            if (rows2.UserID > 0) {
                                ...
                                Notifications.create({
                                    ...
                                    OriginTaskInstanceID:
                                        req.body.OriginTaskInstanceID,
                                    Dismiss:
                                        rows2.UserID == req.body.UserID ||
                                        rows.UserID == rows2.UserID
                                            ? 1
                                            : 0
                                });
                            }
    ...
    //------------------------------------------------------------------------------------------
    router.post("/comments/edit", participantAuthentication, function(
        req,
        res
    ) {
    ...
    //-----------------------------------------------------------------------------
    router.post("/comments/delete", participantAuthentication, function(
        req,
        res
    ) {
        if (req.body.CommentsID == null) {
    ...
    //-------------------------------------------------------------------------
    router.post("/comments/removeFlag", participantAuthentication, function(
        req,
        res
    ) {
        if (req.body.CommentsID == null) {
    ...
    router.post("/comments/rating", participantAuthentication, function(
        req,
        res
    ) {
        if (req.body.CommentsID == null) {
    ...
    //-------------------------------------------------------------------------
    router.get(
        "/comments/countOfComments/:Target/id/:TargetID",
        participantAuthentication,
        function(req, res) {
            console.log(
                "/comments/countOfComments/:Target/id/:TargetID was called"
            );
            Comments.findAll({
                where: {
    ...
    //-------------------------------------------------------------------------
    router.get(
        "/comments/countOfFlags/:Target/id/:TargetID",
        participantAuthentication,
        function(req, res) {
    ...
    //-------------------------------------------------------------------------
    router.get(
        "/comments/countOfUsers/:assignmentInstanceID",
        participantAuthentication,
        function(req, res) {
    ...
    //-------------------------------------------------------------------------
    router.get(
        "/comments/countOfRating/:assignmentInstanceID",
        participantAuthentication,
        function(req, res) {
    ...
    //-------------------------------------------------------------------------
    router.get(
        "/comments/aveRating/comment/:CommentsID",
        participantAuthentication,
        function(req, res) {
    ...
    //-------------------------------------------------------------------------
    router.get(
        "/comments/aveRating/comment/:Target/id/:TargetID",
        participantAuthentication,
        async function(req, res) {
    ...
    //-------------------------------------------------------------------------
    router.get(
        "/comments/aveRating/comment/:userID",
        participantAuthentication,
        function(req, res) {
            console.log("/comments/aveRating/comment/ was called");
            var total = 0.0;
    ...
    //-------------------------------------------------------------------------
    router.get(
        "/comments/ai/:AssignmentInstanceID",
        participantAuthentication,
        function(req, res) {
    ...
    //-------------------------------------------------------------------------
    router.get(
        "/comments/ti/:Target/id/:TargetID",
        participantAuthentication,
        async function(req, res) {
                ...
                for (var i = 0; i < parents.length; i++) {
                    if (parents[i].CommentsID == children[j].Parents) {
                        ...
                        while (
                            parents[i] != null &&
                            i < parents.length &&
                            (parents[i].Parents == children[j].Parents ||
                                parents[i].Parents == parents[m].CommentsID)
                        ) {
                            i++;
                        }
    ...
    router.get(
        ...
        function(req, res) {
            ...
            Comments.findAll({
                ...
                .catch(function(err) {
                    console.log(
                        "comments/CommentsID/:CommentsID " + err.message
                    );
                    res.status(400).end();
                });
        }
    ...
    //-------------------------------------------------------------------------
    router.get(
        "/comments/IDData/:TaskInstanceID",
        participantAuthentication,
        function(req, res) {
            ...
            TaskInstance.findAll({
                ...
                .catch(function(err) {
                    console.log(
                        "/comments/IDData/:TaskInstanceID " + err.message
                    );
                    res.status(400).end();
                });
        }
    ...
    //-------------------------------------------------------------------------
    router.get(
        "/comments/TaskIDData/:WorkflowInstanceID",
        participantAuthentication,
        function(req, res) {
    ...
    //-------------------------------------------------------------------------
    router.get("/comments/userID/:UserID", participantAuthentication, function(
        req,
        res
    ) {
    ...
    //-------------------------------------------------------------------------
    router.get(
        "/comments/courseData/:assignmentInstanceID",
        participantAuthentication,
        async function(req, res) {
            ...
            var AI_Result = await AssignmentInstance.findOne({
                ...
                console.log(
                    "comments/courseData/:assignmentInstanceID AI" + err.message
                );
                res.status(400).end();
            });
            ...
            var Section_Result = await Section.findOne({
                ...
                console.log(
                    "comments/courseData/:assignmentInstanceID Section" +
                        err.message
                );
                res.status(400).end();
            });
            ...
            var Course_Result = await Course.findOne({
                ...
                console.log(
                    "comments/courseData/:assignmentInstanceID Course" +
                        err.message
                );
                res.status(400).end();
            });
            ...
            var Semester_Result = await Semester.findOne({
                ...
                console.log(
                    "comments/courseData/:assignmentInstanceID Semester" +
                        err.message
                );
                res.status(400).end();
            });
    ...
    //-------------------------------------------------------------------------
    router.post("/comments/hide", participantAuthentication, function(
        req,
        res
    ) {
        if (req.body.CommentsID == null) {
    ...
    //-------------------------------------------------------------------------
    router.post("/comments/unhide", participantAuthentication, function(
        req,
        res
    ) {
        if (req.body.CommentsID == null) {
    ...
    //------------------------Contact APIs-------------------------------------
    router.get("/contact/add/:UserID", participantAuthentication, function(
        req,
        res
    ) {
        ...
        User.findAll({
            ...
            .then(function(rows) {
                console.log(
                    "Creating UserID,FirstName,LastName,OrganizationGroup."
                );
                Contact.create({
                    UserID: rows[0].UserID,
    ...
    //---------------------------------------------------------------------------
    router.delete("/contact/delete/:UserID", teacherAuthentication, function(
        req,
        res
    ) {
        Contact.destroy({
    ...
    //---------------------------------------------------------------------------
    router.get(
        "/contact/organizationGroup/:OrganizationGroup",
        participantAuthentication,
        function(req, res) {
            Contact.findAll({
                where: {
    ...
    //---------------------------------------------------------------------------
    router.get("/contact/global/:Global", participantAuthentication, function(
        req,
        res
    ) {
        Contact.findAll({
    ...
    router.get("/VolunteerPool/:UserID", participantAuthentication, function(
        req,
        res
    ) {
        VolunteerPool.findAll({
    ...
    router.get(
        "/getSectionUserRecord/:sectionUserID",
        participantAuthentication,
        async function(req, res) {
    ...
    router.get(
        "/assignment/structure/:assignmentInstanceID",
        participantAuthentication,
        async function(req, res) {
            let structure = [];
    ...
    router.get(
        "/assignment/data/:assignmentInstanceID",
        participantAuthentication,
        async function(req, res) {
            let data = {};
    ...
    router.get("/userProgress/:userID/:categoryID", async function(req, res) {
        ...
        sequelize
            .query(select, {
                replacements: [req.params.userID, req.params.categoryID],
                type: sequelize.QueryTypes.SELECT
            })
            .then(result => {
                if (!result) {
    ...
    router.get("/studentCourses/:userID/:semesterID", async function(req, res) {
        ...
        sequelize
            .query(select, {
                replacements: [req.params.userID, req.params.semesterID],
                type: sequelize.QueryTypes.SELECT
            })
            .then(result => {
                if (!result) {
    ...
    router.get(
        "/badgeCategories/:courseID/:sectionID/:semesterID",
        async function(req, res) {
    ...
    router.get(
        "/getSectionRanking/:semesterID/:courseID/:sectionID/:userID",
        async function(req, res) {
            let lastUpdate = await StudentRankSnapchot.findOne({
                attributes: ["UpdateDate"],
                order: [["StudentRankSnapchotID", "DESC"]]
            });
    ...
    router.get("/getSectionsRanking/:semesterID", async function(req, res) {
        let lastUpdate = await SectionRankSnapchot.findOne({
            attributes: ["UpdateDate"],
            order: [["SectionRankSnapchotID", "DESC"]]
        });
    ...
    router.get(
        "/getLevels/:semesterID/:courseID/:sectionID/:userID",
        async function(req, res) {
    ...
    router.get("/getGoals/:semesterID/:courseID/:sectionID", async function(
        req,
        res
    ) {
    ...
    router.post("/claimExtraCredit", async function(req, res) {
        ...
        await grade.claimExtraCredit(
            req.body.goalInstanceID,
            req.body.sectionUserID
        );
        res.status(200).end();
    });
    ...
    router.get("/notifications/user/:UserID", function(req, res) {
        console.log(
            "/notifications/user/:UserID was called ",
            req.params.UserID
        );
        var count = 0;
        Notifications.findAll({
                ...
                if (rows.length == 0) {
                    ...
                    rows.forEach((i, index, array) => {
                        console.log(
                            ...
                            i.NotificationsID,
                            i.NotificationTarget
                        );
                        if (
                            i.NotificationTarget == "Comment" ||
                            i.NotificationTarget == "Flag"
                        ) {
                            Comments.findOne({
                                where: {
                                ...
                                AssignmentInstance.findOne({
                                    where: {
                                        AssignmentInstanceID:
                                            commentsRows.AssignmentInstanceID
                                    }
                                }).then(function(assignmentInstanceRows) {
                                    Assignment.findOne({
                                        where: {
                                            AssignmentID:
                                                assignmentInstanceRows.AssignmentID
                                        }
                                    }).then(function(assignmentRows) {
                                        TaskInstance.findOne({
                                            where: {
                                                TaskInstanceID:
                                                    commentsRows.TargetID
                                            }
                                        }).then(function(taskInstanceRows) {
                                            TaskActivity.findOne({
                                                where: {
                                                    TaskActivityID:
                                                        taskInstanceRows.TaskActivityID
                                                }
                                            })
                                                .then(function(
                                                    taskActivityRows
                                                ) {
                                                    i.dataValues.TaskName =
                                                        taskActivityRows.DisplayName;
                                                    i.dataValues.AssignmentName =
                                                        assignmentRows.DisplayName;
                                                    i.dataValues.CommentTarget =
                                                        commentsRows.CommentTarget;
                                                    i.dataValues.CommentTargetID =
                                                        commentsRows.TargetID;
                                                })
                                                .then(function(result2) {
                                                    var taskFactory = new TaskFactory();
                                                    i.dataValues.LinkID =
                                                        i.OriginTaskInstanceID;
                                                    taskFactory
                                                        .getNextTask(
                                                            i.OriginTaskInstanceID,
                                                            new Array()
                        ...
                        if (i.NotificationTarget == "VolunteerPool") {
                            VolunteerPool.findOne({
                                ...
                                AssignmentInstance.findOne({
                                    where: {
                                        AssignmentInstanceID:
                                            volunteerPoolRows.AssignmentInstanceID
                                    }
                                }).then(function(assignmentInstanceRows) {
                                    Assignment.findOne({
                                        where: {
                                        ...
                                        Section.findOne({
                                            where: {
                                                SectionID:
                                                    volunteerPoolRows.SectionID
                                            }
                                        }).then(function(sectionRows) {
                                            Course.findOne({
                                                where: {
                                                    CourseID:
                                                        sectionRows.CourseID
                                                }
                                            }).then(function(courseRows) {
                                                User.findOne({
                                                    where: {
                                                        UserID:
                                                            volunteerPoolRows.UserID
                                                    }
                                                    ...
                                                    SectionUser.findOne({
                                                        ...
                                                        i.dataValues.SectionName =
                                                            sectionRows.Name;
                                                        i.dataValues.AssignmentName =
                                                            assignmentRows.DisplayName;
                                                        i.dataValues.CourseName =
                                                            courseRows.Name;
                                                        i.dataValues.Actor = userRows.FirstName.concat(
                                                            " "
                                                        ).concat(
                                                            userRows.LastName
                                                        );
                                                        i.dataValues.ActorID =
                                                            userRows.UserID;
                                                        i.dataValues.OrganizationID =
                                                            courseRows.OrganizationID;
                                                        i.dataValues.CourseID =
                                                            courseRows.CourseID;
                                                        i.dataValues.SemesterID =
                                                            sectionRows.SemesterID;
                                                        i.dataValues.SectionID =
                                                            sectionRows.SectionID;
                                                        i.dataValues.Role =
                                                            sectionUserRows.Role;
                                                        count++;
                                                        if (
                                                            count ==
                                                            array.length
    ...
    router.get("/oldnotifications/user/:UserID", function(req, res) {
        console.log(
            "/oldnotifications/user/:UserID was called ",
            req.params.UserID
        );
        var count = 0;
        Notifications.findAll({
    ...
    //---------------------------------------------------------------------------
    router.post("/notifications/dismiss/:userID/:commentsID", function(
        req,
        res
    ) {
        ...
        Notifications.update(
            ...
            .catch(function(err) {
                console.log(
                    "/notifications/dismiss/:userID/:commentsID" + err.message
                );
                res.status(401).end();
            });
    });
    ...
    router.get(
        "/AssignmentArchive/save/:AssignmentID",
        adminAuthentication,
        function(req, res) {
            ...
            Assignment.findAll({
                ...
                .catch(function(err) {
                    console.log(
                        "/AssignmentArchive/save/:AssignmentInstanceID " +
                            err.message
                    );
                    res.status(400).end();
                });
    ...
    router.get(
        "/AssignmentRestore/save/:AssignmentID",
        adminAuthentication,
        function(req, res) {
            var assignmentArray = new Array();
            Assignment_Archive.findAll({
                ...
                .catch(function(err) {
                    console.log(
                        "/AssignmentRestore/save/:AssignmentInstanceID " +
                            err.message
                    );
                    res.status(400).end();
                });
            ...
            return sequelize
                    ...
                    return AssignmentGrade.findAll({
                        ...
                        for (var x = 0; x < arrayLength; x++) {
                            ArchivedAssignmentGrade.create(
                                {
                                    AssignmentGradeID:
                                        rows[x].AssignmentGradeID,
                                    AssignmentInstanceID:
                                        rows[x].AssignmentInstanceID,
                ...
                .then(function() {
                    return AssignmentGrade.destroy({
                        ...
                        console.log(
                            "Delete AssignmentGrade Success and saved to other back up"
                        );
                    });
                })
                .then(function() {
                            ...
                            ArchivedWorkflowGrade.create(
                                {
                                    ...
                                    WorkflowActivityID:
                                        rows[x].WorkflowActivityID,
                                    SectionUserID: rows[x].SectionUserID,
                                    AssignmentInstanceID:
                                        rows[x].AssignmentInstanceID,
                                    WorkflowInstanceID:
                                        rows[x].WorkflowInstanceID,
                                    Grade: rows[x].Grade,
                                    Comments: rows[x].Comments
                                },
                                {
                ...
                .then(function() {
                    return WorkflowGrade.destroy({
                        ...
                        console.log(
                            "Delete WorkflowGrade Success and saved to other back up"
                        );
                    });
                })
                .then(function() {
                            ...
                            ArchivedTaskGrade.create(
                                {
                                    ...
                                    TAGradeWeightInAssignment:
                                        rows[x].TAGradeWeightInAssignment,
                ...
                .then(function() {
                    return TaskGrade.destroy({
                        ...
                        console.log(
                            "Delete TaskGrade Success and saved to other back up"
                        );
                    });
                })
                .then(function() {
                    return TaskInstance.findAll({
                        ...
                        for (var x = 0; x < arrayLength; x++) {
                            await TaskSimpleGrade.findAll({
                                where: {
                                    TaskInstanceID:
                                        assigninstancerows[x].TaskInstanceID
                                }
                                ...
                                .then(function(rows) {
                                    ...
                                    for (var x = 0; x < arrayLength; x++) {
                                        ArchivedTaskSimpleGrade.create(
                                            {
                                                TaskSimpleGradeID:
                                                    rows[x].TaskSimpleGradeID,
                                                TaskInstanceID:
                                                    rows[x].TaskInstanceID,
                                                SectionUserID:
                                                    rows[x].SectionUserID,
                                                WorkflowActivityID:
                                                    rows[x].WorkflowActivityID,
                                                TaskActivityID:
                                                    rows[x].TaskActivityID,
                                                AssignmentInstanceID:
                                                    rows[x]
                                                        .AssignmentInstanceID,
                                                WorkflowInstanceID:
                                                ...
                                                Grade: rows[x].Grade,
                                                TIExtraCredit:
                                                    rows[x].TIExtraCredit,
                                                Comments: rows[x].Comments,
                                                TADisplayName:
                                                    rows[x].TADisplayName,
                                                WADisplayName:
                                                    rows[x].WADisplayName,
                                ...
                                .then(function() {
                                    return TaskSimpleGrade.destroy({
                                        where: {
                                            TaskInstanceID:
                                                assigninstancerows[x]
                                                    .TaskInstanceID
                                        },
                                        transaction: t
                                    }).then(function(rows) {
                                        console.log(
                                            "Delete TaskSimpleGRade Success and saved to other back up"
                                        );
                                    });
                                });
                        }
                                    ...
                                    ArchivedAssignmentInstance.create(
                                        {
                                            ...
                                            DisplayName:
                                                assigninstancerows[x]
                                                    .DisplayName,
                                            SectionID:
                                                assigninstancerows[x].SectionID,
                                            StartDate:
                                                assigninstancerows[x].StartDate,
                                            EndDate:
                                                assigninstancerows[x].EndDate,
                                            WorkflowCollection: JSON.parse(
                                                assigninstancerows[x]
                                                    .WorkflowCollection
                                            ),
                                            WorkflowTiming: JSON.parse(
                                                assigninstancerows[x]
                                                    .WorkflowTiming
                                            ),
                                            Volunteers:
                                                assigninstancerows[x].Volunteers
                                        },
                                        {
                                            transaction: t
                                        }
                ...
                .then(function() {
                    return TaskInstance.destroy({
                        ...
                        console.log(
                            "Delete TaskInstance Success and saved to other back up"
                        );
                    });
                })
                .then(function() {
                    return WorkflowInstance.destroy({
                        ...
                        console.log(
                            "Delete WorkflowInstance Success and saved to other back up"
                        );
                    });
                })
                .then(function() {
                    return AssignmentInstance.destroy({
                        ...
                        console.log(
                            "Delete AssignmentInstance Success and saved to other back up"
                        );
                    });
                })
                .then(function() {
    ...
    router.get("/archiveinstance/:AssignmentInstanceID", function(req, res) {
        ...
        archiveinstance(req.params.AssignmentInstanceID).then(function() {
            res.status(201).end();
        });
    });
            ...
            return sequelize
                            ...
                            ArchivedWorkflowActivity.create(
                                {
                                    WorkflowActivityID:
                                        rows[x].WorkflowActivityID,
                                    AssignmentID: rows[x].AssignmentID,
                                    TaskActivityCollection: JSON.parse(
                                        rows[x].TaskActivityCollection
                                    ),
                                    Name: rows[x].Name,
                                    Type: rows[x].Type,
                                    GradeDistribution: JSON.parse(
                                        rows[x].GradeDistribution
                                    ),
                                    NumberOfSets: rows[x].NumberOfSets,
                                    Documentation: rows[x].Documentation,
                                    GroupSize: rows[x].GroupSize,
                                    StartTaskActivity:
                                        rows[x].StartTaskActivity,
                                    WorkflowStructure: JSON.parse(
                                        rows[x].WorkflowStructure
                                    ),
                ...
                .then(function() {
                    return Assignment.findAll({
                        ...
                        ArchivedAssignment.create({
                            AssignmentID: rows[0].AssignmentID,
                            OwnerID: rows[0].OwnerID,
                            WorkflowActivityIDs: JSON.parse(
                                rows[0].WorkflowActivityIDs
                            ),
                            Instructions: rows[0].Instructions,
                            Documentation: rows[0].Documentation,
                            GradeDistribution: JSON.parse(
                                rows[0].GradeDistribution
                            ),
                            Name: rows[0].Name,
                            Type: rows[0].Type,
                            DisplayName: rows[0].DisplayName,
                ...
                .then(function() {
                    return TaskActivity.destroy({
                        ...
                        console.log(
                            "Delete Taskactivity Success and saved to other back up"
                        );
                    });
                })
                .then(function() {
                    return WorkflowActivity.destroy({
                        ...
                        console.log(
                            "Delete WorkflowActivity Success and saved to other back up"
                        );
                    });
                })
                .then(function() {
                    return Assignment.destroy({
                        ...
                        console.log(
                            "Delete Assignment Success and saved to other back up"
                        );
                    });
                })
                .then(function() {
                    AssignmentInstance.findAll({
                        ...
                        for (var x = 0; x < arrayLength; x++) {
                            promises.push(
                                archiveinstance(
                                    assigninstancerows[x].AssignmentInstanceID
                                )
                            );
                        }
                    });
    ...
    router.get("/restorearchivedinstance/:AssignmentInstanceID", function(
        req,
        res
    ) {
        var assignmentArray = new Array();
        console.log("Restoreassignmentinstance is called\n");
        restoreArchivedInstance(req.params.AssignmentInstanceID).then(
            function() {
                res.status(201).end();
            }
        );
    ...
    router.get("/restoreremovedinstance/:AssignmentInstanceID", function(
        req,
        res
    ) {
        var assignmentArray = new Array();
        console.log("Restoreassignmentinstance is called\n");
        restoreRemovedInstance(req.params.AssignmentInstanceID).then(
            function() {
                res.status(201).end();
            }
        );
            ...
            return sequelize
                ...
                .then(function() {
                    return ArchivedTaskInstance.findAll({
                        ...
                        .then(
                            ArchivedWorkflowInstance.findAll({
                                where: {
                                    AssignmentInstanceID: AssignInsID
                                },
                                attributes: [
                ...
                .then(function() {
                    return ArchivedAssignmentInstance.destroy({
                        ...
                        console.log(
                            "AssignmentInstance Success and saved to other back up"
                        );
                    });
                })
                .then(function() {
            ...
            return sequelize
                ...
                .then(function() {
                    return RemovedTaskInstance.findAll({
                        ...
                        .then(
                            RemovedWorkflowInstance.findAll({
                                where: {
                                    AssignmentInstanceID: AssignInsID
                                },
                                attributes: [
    ...
    router.get("/removeinstance/:AssignmentInstanceID", function(req, res) {
        ...
        removeInstance(req.params.AssignmentInstanceID).then(function() {
            res.status(201).end();
        });
    });
    ...
    router.get(
        "/AssignmentInstanceArchive/save/:AssignmentInstanceID",
        adminAuthentication,
        function(req, res) {
            var assignmentArray = new Array();
    ...
    router.get(
        "/AssignmentInstanceRestore/save/:AssignmentInstanceID",
        adminAuthentication,
        function(req, res) {
            var assignmentArray = new Array();
    ...
    router.get(
        "/TaskActivityArchive/save/:AssignmentID",
        adminAuthentication,
        function(req, res) {
            var assignmentArray = new Array();
            ...
            TaskActivity.findAll({
                ...
                .catch(function(err) {
                    console.log(
                        "/TaskActivityArchive/save/:AssignmentID " + err.message
                    );
                    res.status(400).end();
                });
    ...
    router.get(
        "/TaskActivityRestore/save/:AssignmentID",
        adminAuthentication,
        function(req, res) {
            var assignmentArray = new Array();
            ...
            TaskActivity_Archive.findAll({
                ...
                .catch(function(err) {
                    console.log(
                        "/TaskActivityRestore/save/:AssignmentID " + err.message
                    );
                    res.status(400).end();
                });
    ...
    router.get(
        "/TaskInstanceArchive/save/:AssignmentInstanceID",
        adminAuthentication,
        function(req, res) {
            var assignmentArray = new Array();
            console.log("TaskInstanceArchive is called\n");
    ...
    //Endpoint to archive task instance table entry by giving  AssignmentInstanceID
    router.get(
        "/TaskInstanceRestore/save/:AssignmentInstanceID",
        adminAuthentication,
        function(req, res) {
            var assignmentArray = new Array();
    ...
    router.get(
        "/WorkflowInstanceArchive/save/:AssignmentInstanceID",
        adminAuthentication,
        function(req, res) {
            var assignmentArray = new Array();
    ...
    router.get(
        "/WorkflowInstanceRestore/save/:AssignmentInstanceID",
        adminAuthentication,
        function(req, res) {
            var assignmentArray = new Array();
    ...
    router.get(
        "/WorkflowActivityArchive/save/:AssignmentID",
        adminAuthentication,
        function(req, res) {
            var assignmentArray = new Array();
    ...
    router.get(
        "/WorkflowActivityRestore/save/:AssignmentID",
        adminAuthentication,
        function(req, res) {
            var assignmentArray = new Array();
    ...
    router.get(
        "/findPreviousTasks/:taskInstanceId",
        teacherAuthentication,
        function(req, res) {
            var allocator = new TaskFactory();

            allocator
                .findPreviousTasks(req.params.taskInstanceId, new Array())
                .then(function(done) {
    ...
    router.get("/manager/checkAssignments", teacherAuthentication, function(
        req,
        res
    ) {
        var manager = new Manager();
            ...
            if (user == null) {
                ...
                user.save()
                    ...
                    .catch(function(error) {
                        ...
                        console.log(
                            "/makeUserAdmin : Error while inserting " +
                                error.message
                        );
                        res.status(400).end();
                    });
    ...
    router.put("/makeUserNotAdmin/", adminAuthentication, function(req, res) {
        UserLogin.find({
            ...
            if (
                userLogin != null &&
                (await password.verify(userLogin.Password, req.body.password))
            ) {
                User.findById(req.body.UserID).then(function(user) {
                    if (user == null) {
                        ...
                        user.save()
                            ...
                            .catch(function(error) {
                                // Ooops, do some error-handling
                                console.log(
                                    "/makeUserNoAdmin : Error while inserting " +
                                        error.message
                                );
                                res.status(400).end();
                            });
    ...
    router.get("/getSubWorkFlow/:taskInstanceID", adminAuthentication, function(
        req,
        res
    ) {
        var taskFactory = new TaskFactory();
        taskFactory
            .getSubWorkflow(req.params.taskInstanceID, new Array())
            .then(function(subworkflow) {
                res.json({
                    Error: false,
    ...
    router.get("/getNextTask/:taskInstanceID", adminAuthentication, function(
        req,
        res
    ) {
        var taskFactory = new TaskFactory();
        taskFactory
            .getNextTask(req.params.taskInstanceID, new Array())
    ...
    //---------------------------------------------------------------------------
    router.get("/userManagement", adminAuthentication, async function(
        req,
        res
    ) {
    ...
    //---------------------------------------------------------------------------
    router.get("/userManagement/blocked/:UserID", adminAuthentication, function(
        req,
        res
    ) {
    ...
    //---------------------------------------------------------------------------
    router.get(
        "/userManagement/unblocked/:UserID",
        adminAuthentication,
        function(req, res) {
    ...
    router.post("/reallocate/user_based", teacherAuthentication, async function(
        req,
        res
    ) {
        if (
            req.body.ai_ids == null ||
            req.body.old_user_ids == null ||
            req.body.is_extra_credit == null ||
            req.body.sec_id == null ||
            req.body.user_pool_wc == null ||
            req.body.user_pool_woc == null
        ) {
        ...
        await Promise.map(req.body.old_user_ids, async old_user_id => {
            if (inactivate_users == "all_assignments") {
                await allocate.inactivate_section_user(
                    req.body.sec_id,
                    old_user_id
                ); // deactive user in section
            }
        ...
        var ais = [];
        if (remove_from_all_assignments) {
            // remove user from all Assigments
            ...
            await Promise.map(req.body.ai_ids, async ai_id => {
                // remove users from provided ais
                var ai = await AssignmentInstance.findOne({
                    where: {
                        AssignmentInstanceID: ai_id
                    }
        ...
        var result = await allocate.reallocate_users(
            req.body.sec_id,
            ais,
            req.body.old_user_ids,
            req.body.user_pool_wc,
            req.body.user_pool_woc,
            req.body.is_extra_credit
        );
        res.json(result);
    });
    ...
    //@ user_pool_woc: [#,..] array of users without constrains
    //@ is_extra_credit: boolean
    router.post("/reallocate/task_based", teacherAuthentication, async function(
        req,
        res
    ) {
        if (
            req.body.taskarray == null ||
            req.body.user_pool_wc == null ||
            req.body.user_pool_woc == null ||
            req.body.is_extra_credit == null
        ) {
        ...
        var allocate = new Allocator([], 0);
        var result = await allocate.reallocate_tasks_based(
            req.body.taskarray,
            req.body.user_pool_wc,
            req.body.user_pool_woc,
            req.body.is_extra_credit
    ...
    router.post(
        "/reallocate/cancel_workflows",
        teacherAuthentication,
        async function(req, res) {
            if (req.body.ai_id == null || req.body.wi_ids == null) {
                logger.log(
                    "error",
                    "/reallocate/cancel_workflows: fields cannot be null"
                );
                res.status(400).end();
                return;
            }
            ...
            await Promise.mapSeries(wi_ids, async wi_id => {
                // group the assigments per Workflow Activity
            ...
            try {
                await Promise.mapSeries(assigment_array, async activity => {
                    result = await allocate.cancel_workflow(
                        req.body.ai_id,
                        activity.wa_id,
                        activity.wi_ids
                    );
                ...
                if (result.Error) {
                    needs_confirmation = true;
                    if (
                        result.wanted_to_cancel_started &&
                        !result.extra_task_for_extra_credit
                    ) {
                        Message =
                            "Some users will have less tasks then others, Started workflows will not be cancelled";
                    } else if (
                        result.wanted_to_cancel_started &&
                        result.extra_task_for_extra_credit
                    ) {
                        Message =
                            "Some users will have less tasks then others, extra tasks will be allocated for extra credit, Started workflows will not be cancelled";
                    } else if (
                        !result.wanted_to_cancel_started &&
                        result.extra_task_for_extra_credit
                    ) {
                        Message =
                            "Some users will have less tasks then others, extra tasks will be allocated for extra credit";
                    } else {
                    ...
                    if (
                        ...
                        Message =
                            "Extra tasks were allocated for extra credit, Started workflows were not be cancelled";
                    } else if (
                        !result.wanted_to_cancel_started &&
                        result.extra_task_for_extra_credit
                    ) {
                ...
                if (!needs_confirmation) {
                    // Use the graph and apply it to the database
                    await Promise.map(array_of_results, async w_activity => {
                        ...
                        if (wi_ids.length > 0) {
                            result = await allocate.apply_cancellation_graph(
                                Graph,
                                wi_ids,
                                users_to_realocate,
                                ai_id,
    ...
    router.post(
        "/reallocate/confirm_cancellation",
        teacherAuthentication,
        async function(req, res) {
            if (req.body.data == null) {
                logger.log(
    ...
    router.post("/task/cancel", teacherAuthentication, async function(
        req,
        res
    ) {
        logger.log("info", {
        ...
        await TaskInstance.find({
            ...
            .then(async function(ti) {
                ...
                if (
                    ti_status[0] == "bypassed" ||
                    ti_status[0] == "complete" ||
    ...
    router.post("/task/bypass", teacherAuthentication, async function(
        req,
        res
    ) {
        logger.log("info", {
        ...
        if (
            ...
            logger.log(
                "info",
                "updating TaskInstanceID:",
                req.body.ti_id,
                "to bypassed"
            );
            await TaskInstance.update(
                {
                    // update task before triggering
            ...
            try {
                if (ti.TaskActivity.Type === "edit") {
                    // edit tasks always have [] as next task, treat differently
                ...
                // error with missing grades occurs sometimes
    ...
    router.post(
        "/inactivate/users_in_assignment",
        teacherAuthentication,
        async function(req, res) {
            if (
                req.body.user_ids == null ||
                req.body.ai_id == null ||
                req.body.sec_id == null ||
                req.body.inactivate_users == null
            ) {
                logger.log("error", "/task/cancel");
                res.status(400).end();
                return;
    ...
    router.get("/ModelTest/:userID", function(req, res) {
        WorkflowInstance.findById(req.params.userID).then(function(
            WorkflowInstance
        ) {
            console.log("WorkflowInstance Found");

            WorkflowInstance.getWorkflowActivity().then(function(
                workflowActivity
            ) {
        ...
        WorkflowActivity.findById(req.params.userID).then(function(
            workflowActivity
        ) {
            console.log("WorkflowActivity Found " + workflowActivity.Name);
    ...
    router.post("/inactiveuser/section", teacherAuthentication, function(
        req,
        res
    ) {
        if (req.body.UserID == null) {
    ...
    router.post("/testuser/create", async function(req, res) {
        ...
        var f = await TestUser.findAll({}).catch(function(err) {
    ...
    router.post(
        "/sectionUsers/changeVolunteer/:UserID/:SectionID/:Status/:UserType",
        (req, res) => {
            /** TODO:  This API does a simple database update, but it may need
             * to do some special reallocation to deal with inactive students
             */
            console.log(
                "/sectionUsers/changeVolunteer/ was called",
                req.params.UserID,
                req.params.SectionID,
                req.params.Status
            );
            SectionUser.findOne({
                ...
                .catch(err => {
                    console.log(
                        "error",
                        "post: /sectionUser/changeVolunteer/, user volunteer status not set",
                        {
                            error: err,
                            req_params: req.params
                        }
    ...
    router.get("/SectionUsers/Volunteer/:UserID/:SectionID/:Student", function(
        req,
        res
    ) {
        console.log(
            "/SectionUsers/Volunteer/ was called",
            req.params.UserID,
            req.params.SectionID
        );
        SectionUser.findAll({
            ...
            if (req.params.Student == "true") {
                console.log(
                    "starting dismiss sequence with SectionUserID ",
                    rows[0].SectionUserID,
                    "UserID ",
                    req.params.UserID
                );
                Notifications.findAll({
    ...
    //---------------------------------------------------------------------------
    router.get("/VolunteerPool/:UserID/:SectionID/:Student", async function(
        req,
        res
    ) {
        console.log(
            "/VolunteerPool/:UserID/:SectionID was called",
            req.params.UserID,
            req.params.SectionID,
            req.params.Student
        );
        ...
        for (var i = 0; i < A.length; i++) {
            ...
            for (var j = 0; j < VP.length; j++) {
                if (A[i].AssignmentInstanceID == VP[j].AssignmentInstanceID) {
                    ...
                    result.push({
                        AssignmentInstanceID: A[i].AssignmentInstanceID,
                        DisplayName: A[i].Assignment.DisplayName,
                        Status: VP[j].status,
                        VolunteerPoolID: VP[j].VolunteerPoolID
                    });
                    found = true;
                    if (req.params.Student == "true") {
    ...
    router.post("/VolunteerPool/add", function(req, res) {
        ...
        if (
            req.body.UserID === null ||
            req.body.SectionID ===
                null /*|| req.body.AssignmentInstanceID === null*/
        ) {
        ...
        VolunteerPool.create({
            ...
            .then(function(rows) {
                SectionUser.findAll({
                    ...
                    console.log(
                        "add VolunteerPool success, new ID=",
                        rows.VolunteerPoolID
                    );
                    for (let i of rows2) {
                        Notifications.create({
                            UserID: i.UserID,
    ...
    router.post("/VolunteerPool/appoint", function(req, res) {
        console.log(
            "/VolunteerPool/appoint : was called",
            req.body.userId,
            req.body.SectionID,
            req.body.AssignmentInstanceID,
            req.body.UID
        );
    ...
    router.post("/VolunteerPool/individualStatusUpdate/", async function(
        req,
        res
    ) {
        ...
        VolunteerPool.findOne({
            ...
            .catch(function(err) {
                console.log(
                    "/VolunteerPool/individualStatusUpdate " + err.message
                );
                res.status(401).end();
            });
    });
